...
**触发方式**：脚本本身不带定时能力，靠服务器宝塔面板的"计划任务"每日调用一次（`node src/scripts/purgeDeletedAccounts.js`）。这个配置只存在于服务器上，不在代码仓库里——选择宝塔计划任务而不是代码内置定时器（比如 `node-cron`），是因为项目已经用宝塔管理服务器，不需要再多引入一个定时任务框架、多一个需要保活的常驻进程。**代价**：这个依赖对代码不可见，服务器重装或迁移后容易被遗忘，所以在 `README.md` 里专门记了一笔提醒。

**失败隔离**：每个用户的四步操作（归档用户名、改名、归档凭证、删凭证）在各自独立的事务里执行，互不影响——某个用户处理失败只回滚这一个用户的事务，记录错误后继续处理下一个，不会因为一个用户的问题中断整批清理。脚本最后输出成功/失败的数量和失败用户的 id 列表。

**可重复执行**：脚本靠查询"该用户在 `user_identities` 里是否还有记录"来判断是否已经处理过——处理流程的最后一步就是删除这条记录，所以还没处理过的用户一定还有记录，处理完的一定没有。没有用 `username` 是否带 `_deleted_` 后缀这种字符串特征来判断，因为原用户名本身理论上就可能包含这几个字符，字符串匹配不够可靠；直接看"该做的数据变更是否已经发生"更准确。
